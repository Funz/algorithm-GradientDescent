...
#tags: optimization
#options: yminimization='false'; iterations=100; .delta=1; .epsilon=0.01; .target=Inf; x0=.5
...
getInitialDesign <- function(algorithm,input,output) {
  ...
  d = length(input)
  if (is.na(algorithm$x0)) algorithm$x0=runif(d)
  x = askfinitedifferences(rep_len(algorithm$x0,length.out = d),algorithm$epsilon)
  names(x) <- names(input)
